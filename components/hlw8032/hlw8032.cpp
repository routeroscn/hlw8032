#include "HLW8032.h"
 ...
 #这是一个预处理指令，用于在编译时根据日志级别进行条件编译。只有当日志级别为 VERY_VERBOSE 或更高时，才会编译其中的代码
   ...
   if (header1 == 0xAA) {
     ESP_LOGE(TAG, "HLW8032 not calibrated!");
     return;
   }
   ...
   if ((header1 & 0xF0) == 0xF0) {
     if (header1 & 0xD) {
       ESP_LOGE(TAG, "HLW8032 reports abnormal external circuit or chip damage: (0x%02X)", header1);
       if (header1 & (1 << 3)) {
   ...
   float voltage = 0.0f;
   if (have_voltage) {
     voltage = voltage_coeff / float(voltage_cycle) * 1.98;
   ...
   if (have_current) {
     ...
     if (calculated_current > 0.05f) {
       current = current_coeff / float(current_cycle) * 1.0 / (0.00124 *1000.0);
     }
